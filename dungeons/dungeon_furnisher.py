...
with open('data/meroe/rooms.yaml', 'r') as f:
    dungeon_rooms = {}
    dungeon_tags = {}
    secret_passages = {}
    extra_details = {}
...
class DungeonFurnisher:
    def __init__(self, purpose, theme, random_state=None):
        ...
        self.special_furnishings = [f(random_state=self.random_state) for f in [Window]]

    def furnish(self, layout):
        ornaments = Ornamentation(self.purpose, self.theme, random_state=self.random_state)
        for n, room in layout.nodes(data=True):
            room_type = self.choose_room_type(room['tags'])
            room['description'] = RoomDescription(room_type['description'], ornaments=ornaments)
            room['name'] = room_type['name']
...
class Window(SpecialFurnishing):
    def get_description(self):
        base = self.random_state.choice(special_furnishings_data['window']['templates'])
        vista = self.random_state.choice(special_furnishings_data['window']['vistas'])
        return " ".join([base, vista])

    def appropriate(self, layout):
        return layout.purpose == "prison"
