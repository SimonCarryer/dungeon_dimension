...
class DungeonPopulator:
    ...
    def get_treasure(self, shares=1):
        return self.treasure_source.get_treasure(shares, treasure_type=self.monster_set)
...
class OriginalInhabitants(DungeonPopulator):
    ...
    def populate_room(self, room_id, data):
        ...
        if 'trap' in data['tags'] and 'important' in data['tags']:
            ...
            trap = self.get_trap()
        elif 'important' in data['tags'] and self.roll(5):
            encounter = self.get_encounter(style='leader', difficulty='hard')
        elif 'secret' in data['tags'] and self.roll(5):
            encounter = self.get_encounter(style='elite', occurrence='rare')
        elif 'guarded' in data['tags'] and self.roll(3):
...
class Explorers(DungeonPopulator):
    ...
    def populate(self, layout):
        ...
        for node, data in nodes:
            ...
            elif 'entrance' in data.get('tags'):
